...
void printBoard2(int board[N][N],int pause=100) {
    TermPrint::clear();
    TermGame::sleep(500);
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++)
            if (board[i][j]) {
                TermPrint::print(to_string(board[i][j]) + " ", TermPrint::RED);
            } else {
                TermPrint::print(to_string(board[i][j]) + " ", TermPrint::GREEN);
            }
...
bool checkSolution(int pause=100) {
    int board[N][N];
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
