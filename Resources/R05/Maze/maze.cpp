...
public:
    ...
    Maze(int rows = 16, int cols = 16)
    {
        ...
        __build_maze(true); // Uses stack to randomly visit cells and knockdown walls
    ...
    void traverseMaze(bool pm=true)
    {
        ...
        while (move_stack.size() > 0 && found == false)
        {
            ...
            if (moves.size())
            {
                ...
                //best_index = __best_choice(moves);
...
private:
    ...
    int __best_choice(vector<Cell*> moves){
        int min=99999;
        int d=0;
        int i=0;
        for(int j=0;j<moves.size();j++){
            d = __taxicab_distance(moves[j], exitCell);
            if(d < min){
                min = d;
                i = j;
            }
