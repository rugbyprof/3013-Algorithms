 ...
 *       In this type of maze representation, each cell contains its left wall
 * and ceiling. It's neighboring cells will hold the right wall and floor. It is
 * confusing, but prints out much more compactly on the standard screen.
 */
struct Cell {
...
class Maze {
   ...
   */
   Maze(int rows = 16, int cols = 16) {
      ...
      __build_maze(
          true);      // Uses stack to randomly visit cells and knockdown walls
   ...
   */
   void printMaze() {
      ...
      for (int r = 0; r < height; r++) {
         ...
         for (int c = 0; c < width; c++) {
            cout << maze[r][c]->left << termcolor::red << maze[r][c]->character
                 << termcolor::green;
         }
         cout << "|" << endl;
   ...
   *      Determines direction moved going from cell (r1,c1) => (r2,c2). Used to
   * determine which wall to knock down when building the maze. Returns: string
   */
   ...
   void __build_maze(bool pm = false) {
      ...
      while (move_stack.size() > 0) {
         ...
         if (moves.size()) {
            ...
            d = __get_direction(current->row, current->col, neighbor_row,
                                neighbor_col);
   ...
   *
   *       Creates a Vector of Cells that holds references to unvisited
   * neighbors Params: int row : target cell row int col : target cell col
   * Returns:
